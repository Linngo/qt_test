...
//{
//    unsigned char n=BLOCK_SIZE/4;  // n=9
//    uint8_t *v= buf;
//    uint8_t *k = key;
//    uint8_t z = v[n - 1],y = v[0],sum = 0,e ; //z=0 y=0f
//    uint8_t p,q ;
//    // Coding Part

//    q = S_LOOPTIME + 52 / n ; //q=57/9=6
//    while ( q-- > 0 )
//    {
//        sum += DELTA ;
//        e = sum >> 2 & 3 ;
...
//    QString test;
    ...
    else
    {
        ...
        for(i=0;i<totalBytes/data_size;i++){
            ...
            btea_encrypt(tmp, TEA_key);
            file_bin.write(data);
            qDebug() << data.toHex();
  //          btea_decrpyt(tmp, TEA_key);
  //          qDebug() << data.toHex();
            file_bin.flush();
        }
        data.clear();
    ...
    file_read.close();
//    btea_encrypt(tmp, TEA_key);
//    ttt = QByteArray::fromRawData((char *)data,sizeof (data));
//    qDebug() << ttt.toHex();
